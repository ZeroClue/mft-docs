...
Manage labels (key/value metadata) on a registered agent. All `label`
subcommands require you to be logged in (`mftctl login`).

```bash
mftctl agents label [command] [arguments] [flags]
```

#### agents label list

List all labels for an agent.

```bash
mftctl agents label list [agent-id] [flags]

Arguments:
  [agent-id]   ID of the agent (required)
...
Add or update a label on an agent. Setting a label whose key already exists
updates its value.

```bash
mftctl agents label add [agent-id] [key] [value] [flags]
...
Update the value of an existing label on an agent.

```bash
mftctl agents label update [agent-id] [key] [value] [flags]

Arguments:
  [agent-id]   ID of the agent (required)
  [key]        Label key (required)
  [value]      New label value (required)
...
```bash
mftctl agents label remove [agent-id] [key] [flags]

Arguments:
  [agent-id]   ID of the agent (required)
  [key]        Label key to remove (required)
